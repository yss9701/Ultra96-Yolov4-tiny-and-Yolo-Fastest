...
from nets.yolo_fastest import yolo_body
...
if __name__ == "__main__":
    ...
    anchors_path = 'model_data/yolo_anchors_320.txt'
    # 预训练模型的位置
    weights_path = 'logs_12/ep075-loss8.043-val_loss7.786.h5'
    # 获得classes和anchor
    ...
    #model_complexity_param = model_complexity.ModelParametersCallback(log_dir, verbose=1)
    ...
    if False:
        ...
        model.fit(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
                  ...
                  callbacks=[logging, checkpoint, reduce_lr, early_stopping, model_complexity_param])
        model.save_weights(log_dir + 'last1.h5')
        # pruner.export_model(model_path='pruned_vgg19_cifar10.h5', mask_path='mask_vgg19_cifar10.h5')
    ...
    validation_generator = data_generator(lines[num_train:], batch_size, input_shape, anchors, num_classes, mosaic=False)
    ...
    for layer in model.layers[start:end]:
        if layer.__class__.__name__ == 'Conv2D':
            print(layer.name)
        #    if layer.name == 'conv2d_17':
        #        a = pruner.calc_mask(layer, configure_dict)
    model = pruner.compress_model()
    #prun_a = pruner.compress_model()
    #min_gm_kernels = sorted(prun_a, key=lambda x: x[0])[:10]
    #min_gm_kernels_1 = [x[1] for x in min_gm_kernels]
    #surgeon = Surgeon(model, copy=False)
    #model = pruner.compress_model_1(channels_p=min_gm_kernels_1)
    #a=1
    ...
    model.save_weights(output_dir + '1' + '.h5')
    ...
    if False:
        ...
        with open(r'F:\yolo\yolo_fastest_tf2\inception_flowers\model_config.json', 'r') as file:
            model_json1 = file.read()
        new_model = model_from_json(model_json1)
        new_model.load_weights(output_dir + '1' + '.h5', by_name=True, skip_mismatch=True)
        ...
        # 输入为*model_body.input, *y_true
        # 输出为model_loss
        loss_input = [*new_model.output, *y_true]
        model_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
                            arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.5,
                                       'label_smoothing': label_smoothing})(loss_input)

        model = Model([new_model.input, *y_true], model_loss)
        ...
        #model.save_weights(output_dir + '1' + '.h5')
        #json_config = model.to_json()
        #with open(output_dir + 'model_config.json', 'w') as json_file:
        #    json_file.write(json_config)
        #model.save(output_dir + '1' + '.h5')
        #del model
        #tf.python.keras.backend.clear_session()
        #ops.reset_default_graph()
        #model = load_model(output_dir + '1' + '.h5')
        #with open(r'F:\yolo\yolo_fastest_tf2\inception_flowers\model_config.json', 'r') as file:
        #    model_json1 = file.read()
        #new_model = model_from_json(model_json1)
        #new_model.load_weights(output_dir + '1' + '.h5')
